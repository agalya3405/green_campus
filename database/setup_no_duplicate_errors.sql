...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'green_innovation' AND TABLE_NAME = 'ideas' AND COLUMN_NAME = 'assigned_faculty_id') THEN
        ALTER TABLE ideas ADD COLUMN assigned_faculty_id INT DEFAULT NULL;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'green_innovation' AND TABLE_NAME = 'ideas' AND COLUMN_NAME = 'faculty_remarks') THEN
