...
// Check if ideas table has assigned_staff_id (Staff module)
    ...
    if ($has_staff_id && !empty($staff_list)) {
        ...
        if ($assigned_staff_id > 0 && $staff_name !== '') {
            ...
            if (mysqli_stmt_execute($stmt)) {
                mysqli_stmt_close($stmt);
                header('Location: admin/admin_dashboard.php?success=1&msg=' . urlencode('Idea assigned to staff successfully.'));
                exit;
            }
        ...
        if (empty($assigned_to)) {
            $error = 'Please enter staff name to assign.';
        } else {
            if ($has_staff_id) {
...
<body>
    ...
    <main class="container">
        <h1>Assign Idea</h1>
        <p class="subtitle">Assign idea to a staff member. Staff will see it on their dashboard.</p>
        ...
        <div class="card">
            ...
            <form method="POST" action="assign_idea.php?id=<?php echo (int) $id; ?>">
                ...
                <?php else: ?>
                <div class="form-group">
                    <label for="assigned_to">Assign to (Staff name)</label>
                    <input type="text" id="assigned_to" name="assigned_to" required placeholder="Enter staff name" value="<?php echo htmlspecialchars($idea['assigned_to'] ?? $_POST['assigned_to'] ?? ''); ?>">
                </div>
                <?php endif; ?>
