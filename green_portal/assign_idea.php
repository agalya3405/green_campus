...
$stmt = mysqli_prepare($conn, "SELECT id, title, status, assigned_to, " . ($has_faculty_id ? "assigned_faculty_id" : "NULL AS assigned_faculty_id") . " FROM ideas WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($has_faculty_id && !empty($faculty_list)) {
        $assigned_faculty_id = isset($_POST['assigned_faculty_id']) ? (int) $_POST['assigned_faculty_id'] : 0;
        $faculty_name = '';
        foreach ($faculty_list as $s) {
            if ((int) $s['id'] === $assigned_faculty_id) {
                $faculty_name = $s['name'];
                break;
            }
        }
        if ($assigned_faculty_id > 0 && $faculty_name !== '') {
            $stmt = mysqli_prepare($conn, "UPDATE ideas SET assigned_faculty_id = ?, assigned_to = ?, status = IF(status = 'Pending', 'Approved', status) WHERE id = ?");
            mysqli_stmt_bind_param($stmt, "isi", $assigned_faculty_id, $faculty_name, $id);
    ...
    <main class="container">
        ...
        <div class="card">
            ...
            <form method="POST" action="assign_idea.php?id=<?php echo (int) $id; ?>">
                <?php if ($has_faculty_id && !empty($faculty_list)): ?>
                <div class="form-group">
                    <label for="assigned_faculty_id">Assign to (Faculty)</label>
                    <select id="assigned_faculty_id" name="assigned_faculty_id" required>
                        <option value="">— Select faculty —</option>
                        <?php foreach ($faculty_list as $s): ?>
                            <option value="<?php echo (int) $s['id']; ?>" <?php echo (isset($idea['assigned_faculty_id']) && (int) $idea['assigned_faculty_id'] === (int) $s['id']) ? 'selected' : ''; ?>>
