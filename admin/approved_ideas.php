...
// Fetch only Approved ideas (submitter = users via user_id, faculty = users via assigned_staff_id)
$query = "SELECT i.*,
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if (empty($row['assigned_staff_id'])): ?>
                                                <button type="button" onclick="openAssignModal(<?php echo (int) $row['id']; ?>)" class="btn btn-sm btn-approve" style="background-color: var(--primary-color); color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">✓ Assign Faculty</button>
                                            <?php else: ?>
